...
var webapp_nsp = io.of('/android')				//namespace của webapp
...
console.log("Server nodejs chay tai dia chi: " + myip.getLocalIP4() + ":" + PORT)
...
webapp_nsp.on('connection', function(socket) {
	...
	socket.on('disconnect', function() {
		console.log("Disconnect socket android")
	})
	
	socket.on('nothing', function(packet) {
		console.log("android rev and send to esp8266 packet: ", packet.data) //in ra để debug
		var eventName = packet.data[0]
		var eventJson = packet.data[1] || {} //nếu gửi thêm json thì lấy json từ lệnh gửi, không thì gửi chuỗi json rỗng, {}
		esp8266_nsp.emit(eventName, eventJson) //gửi toàn bộ lệnh + json đến esp8266
	});
	socket.on('FAN',function (packet) {
		console.log("android rev and send to esp8266 packet: ", packet) //in ra để debug
		var eventName = 'FAN'
		var eventJson = packet|| {} //nếu gửi thêm json thì lấy json từ lệnh gửi, không thì gửi chuỗi json rỗng, {}
		esp8266_nsp.emit(eventName, eventJson)
	});
	socket.on('LED 1',function (packet) {
		console.log("android rev and send to esp8266 packet: ", packet) //in ra để debug
		var eventName = 'LED 1'
		var eventJson = packet|| {} //nếu gửi thêm json thì lấy json từ lệnh gửi, không thì gửi chuỗi json rỗng, {}
		esp8266_nsp.emit(eventName, eventJson)
	});
	socket.on('LED 2',function (packet) {
		console.log("android rev and send to esp8266 packet: ", packet) //in ra để debug
		var eventName = 'LED 2'
		var eventJson = packet|| {} //nếu gửi thêm json thì lấy json từ lệnh gửi, không thì gửi chuỗi json rỗng, {}
		esp8266_nsp.emit(eventName, eventJson)
